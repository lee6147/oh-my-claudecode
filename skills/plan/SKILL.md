...
- **On completion**: Call `state_clear(mode="ralplan")` after the workflow finishes (Critic approval, user rejection, max iterations reached, or handoff to execution via ralph/team)
- **On error/abort**: Call `state_clear(mode="ralplan")` before stopping

Without cleanup, the stop hook blocks all subsequent stops with `[RALPLAN - CONSENSUS PLANNING]` reinforcement messages even after the consensus workflow has finished.
...
7. On Critic approval (with improvements applied): *(--interactive only)* If running with `--interactive`, use `AskUserQuestion` to present the plan with these options:
   ...
   If NOT running with `--interactive`, output the final approved plan, call `state_clear(mode="ralplan")`, and stop. Do NOT auto-execute.
8. *(--interactive only)* User chooses via the structured `AskUserQuestion` UI (never ask for approval in plain text). If user selects **Reject**, call `state_clear(mode="ralplan")` and stop.
9. On user approval (--interactive only): Call `state_clear(mode="ralplan")` **before** invoking the execution skill (ralph/team), so the stop hook does not interfere with the execution mode's own enforcement.
...
- When user selects "Clear context and implement" in step 7 (--interactive only): call `state_clear(mode="ralplan")` first, then invoke `Skill("compact")` to compress the accumulated planning context, then immediately invoke `Skill("oh-my-claudecode:ralph")` with the plan path -- the compact step is critical to free up context before the implementation loop begins
- **CRITICAL — Consensus mode state lifecycle**: Always call `state_clear(mode="ralplan")` before stopping or handing off to execution. Failing to clear the state causes the persistent-mode stop hook to block all subsequent stops indefinitely.
</Tool_Usage>
...
- In consensus mode, stop after 5 Planner/Architect/Critic iterations and present the best version. Call `state_clear(mode="ralplan")` when presenting the final version.
- Consensus mode without `--interactive` outputs the final plan and stops; with `--interactive`, requires explicit user approval before any implementation begins. **Always** call `state_clear(mode="ralplan")` before stopping.
- If the user says "just do it" or "skip planning", call `state_clear(mode="ralplan")` then **MUST** invoke `Skill("oh-my-claudecode:ralph")` to transition to execution mode. Do NOT implement directly in the planning agent.
...
- [ ] In consensus mode: `state_clear(mode="ralplan")` called on every exit path (approval, rejection, max iterations, error, handoff to execution)
</Final_Checklist>
